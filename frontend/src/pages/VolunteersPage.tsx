  ...
  return (
    <div className="h-full bg-transparent p-6 overflow-y-auto custom-scrollbar">
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-[1.6rem] font-bold text-inherit">Volunteer Roster</h1>
      ...
      <div className="flex items-center space-x-4 mb-8">
        <input 
          ...
          className="bg-white/5 border border-white/10 rounded-lg px-4 py-2.5 text-inherit w-80 focus:outline-none focus:border-blue-500/50 transition-all text-sm"
        />
        <div className="flex bg-white/5 p-1 rounded-lg border border-white/10">
          {["All", "Available", "Busy", "Medical", "Engineering", "Food"].map(f => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-md text-xs font-semibold transition-all ${filter === f ? 'bg-blue-600 text-white shadow-lg' : 'text-gray-400 hover:text-blue-400'}`}
            >
              {f}
            </button>
          ))}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-20">
        {filteredVolunteers.map(v => (
          <div key={v.id} className="bg-[#161B22] border border-gray-800 rounded-2xl p-6 hover:border-blue-500/50 hover:-translate-y-1 transition-all duration-200 group shadow-lg">
            <div className="flex items-center space-x-4 mb-4">
              ...
              <div>
                <h3 className="font-bold text-gray-100 group-hover:text-blue-400 transition-colors">{v.name}</h3>
                <p className="text-[0.8rem] text-gray-400 flex items-center">
                  <span className="mr-1 opacity-60">📍</span> {v.city}
                </p>
              </div>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {v.skills.map(s => (
                <span key={s} className="px-2 py-0.5 bg-blue-500/10 text-blue-400 border border-blue-500/20 rounded text-[0.65rem] font-bold uppercase tracking-wider">
                  {s}
                </span>
              ))}
            </div>
            ...
            <div className="space-y-3 mb-6">
              <div className="flex justify-between items-center mb-1">
                <span className="text-[0.6rem] text-gray-500 font-bold uppercase tracking-[0.1em]">Impact Hours</span>
                <span className="text-[0.7rem] text-gray-100 mono font-bold">{v.hours}/20h</span>
              </div>
              <div className="w-full h-1.5 bg-gray-800 rounded-full overflow-hidden">
                <div 
                  className={`h-full rounded-full transition-all duration-500 ${v.hours >= 18 ? 'bg-red-500' : 'bg-green-500'}`}
                  style={{ width: `${(v.hours / 20) * 100}%` }}
                ></div>
              </div>
              <div className="flex justify-between items-center pt-1">
                <span className="text-[0.8rem] text-gray-400 mono flex items-center">
            ...
            <button 
              onClick={() => navigate('/')}
              className="w-full py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-xl font-bold text-sm transition-all shadow-lg shadow-blue-600/10 flex items-center justify-center gap-2"
            >
              Dispatch Resource <span>→</span>
            </button>
          </div>
        ))}
      </div>
